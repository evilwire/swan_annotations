...
$\Cat{C}$-isos. Set $G(f) = (Tu_B)^{-1}F(f)Tu_A$. First, consider
...
\end{diagram}
\]
and since $Fu_A$ and $Fu_B$ are isomorphisms, we have $GTf = 
Fu_BGf(Fu_A)^{-1} = Ff$. Similar argumens show that $GTf = Ff$
for any $f \in \Cat{A}$.
\end{proof}
