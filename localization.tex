...
\end{diagram}
\]
It is easy to see (via universal properties or by appealing to the 
Embedding Theorem) that
\[
\rel{T} \comp (\rel{S} \comp \rel{R}) = (\rel{T} \comp \rel{S}) 
\comp \rel{R}.
\]
...
$\rel{Id}_A$ (and sometimes just $\rel{Id}$ if $A$ is understood). 
