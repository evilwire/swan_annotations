...
\end{diagram}
\]
where $i: K \to A$ is a mono, and $(Tf)j = 0$ in $\AC$. We proceed 
by showing that $C$ is isomorphic to $TD$ for some $D$ where $D$ 
emits a map to $A$ whose composition with $f$ is 0.
...
\begin{lem}
Let $T$ be as above ($T: \Cat{A} \to \AC$). Then $T$ preserves 
monos.
\end{lem}
\begin{proof}
\end{proof}
